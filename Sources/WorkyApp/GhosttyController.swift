...
struct GhosttyController: GhosttyControlling {
    ...
    func openOrFocus(projectName: String, worktreeName: String, worktreePath: String) {
        ...
        guard let resourcePath = Bundle.main.resourcePath else {
            print("Worky Ghostty: ERROR - Could not find app bundle resource path")
            return
        }
        ...
        print("Worky Ghostty: running script: \(command.joined(separator: " "))")
        print("Worky Ghostty: base color RGB(\(Self.baseColor.r),\(Self.baseColor.g),\(Self.baseColor.b)) + accent RGB(\(accentR),\(accentG),\(accentB)) = tinted RGB(\(tintedR),\(tintedG),\(tintedB))")
