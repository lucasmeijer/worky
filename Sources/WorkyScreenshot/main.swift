...
func captureWindow(windowId: CGWindowID, outPath: String) throws {
    guard let image = CGWindowListCreateImage(
        ...
        throw NSError(domain: "WorkyScreenshot", code: 2, userInfo: [NSLocalizedDescriptionKey: "Failed to capture window image"])
    }

    let rep = NSBitmapImageRep(cgImage: image)
    guard let data = rep.representation(using: .png, properties: [:]) else {
        throw NSError(domain: "WorkyScreenshot", code: 3, userInfo: [NSLocalizedDescriptionKey: "Failed to encode PNG"])
    }
